...
function Destination({arrayDesInit,arrayDesSlug}) {
      ...
      <div className='relative z-10'>
        ...
        <div className='mx-[8.12vw] max-md:mx-[4.27vw] flex filter-blog gap-[2.5vw]' ref={eleRef}>
          <div className='flex flex-col'>
            <span className='text-[0.875vw] text-[#9B9B9B] uppercase font-[400] max-md:hidden'>Destination</span>
            <div className='flex items-center'>
              <Image src={icon} width={20} height={20} className='w-[1.875vw] h-[1.875vw] max-md:w-[3.73vw] max-md:h-[3.73vw]'/>
              <select name="" id="" className='w-max' onChange={handleChange}>
        ...
        <div className='flex md:gap-[0.75vw] gap-[3.2vw] justify-center items-center relative md:mt-[4.5vw] mt-[8.53vw]'>
          {Array.from({ length: totalPage }, (_, index) => (
            <div
              ...
              className={`cursor-pointer md:w-[2.125vw] md:h-[2.125vw] w-[9.07vw] h-[9.07vw] rounded-[50%] flex justify-center items-center bg-[#171717] ${
                activePage === index ? 'bg-[#171717]  opacity-[1]' : 'bg-[#FFD220] opacity-[0.1]'
              }`}
            >
              <span className={`${activePage === index ? 'text-white' : 'text-[#171717]'} font-[500] text-[1vw] max-md:text-[4.26vw]`}>{index + 1}</span>
            </div>
          ))}
        </div>
      </div>
      <div className='absolute inset-0 max-md:hidden'>
        <Image src={bgBlog} width={1000} height={1000} className='w-full h-full'/>
      </div>
    </div>
  )
}
