...
const list = [{name: 'Information', id: 1, to: ''}, {name: 'Map', id: 2, to: '#mapId'}
, {name: 'Trip detail', id: 3, to: '#tourId'}, {name: 'Book online', id: 4, to: '#bookingId'}]
const OverviewMb = () => {
  ...
  return (
    <div className='hidden max-md:flex flex-col ml-[3.62vw] z-10 mt-[100vh]'>
      <div className='overflow-auto mb-[4.26vw]'>
        <div className='flex gap-[3.2vw] mb-[1.6vw] w-[100vw] '>
          {list?.map(item => 
          (<Link href={`/tour/${param.slug}${item.to}`} 
          className={`text-[3.46vw] h-[6.4vw] pb-[1.6vw] w-[24.42vw] font-poppins text-center
          ${select === item?.id ? 'text-[#B34B1E] font-semibold border-b-[0.4vw] border-[#B34B1E]' : 'font-normal text-[#898989]'}`} 
          key={item.id}>{item?.name}</Link>))}
        </div>
      </div>
      <div className={`${show ? '' : 'h-[40vw] overflow-hidden'}`}>
        {
          data?.map(item => item.id === select ? 
          <div>
            {item?.data?.map(el => (
              <div>
                  <h2 className='text-[3.46vw] flex gap-[1.6vw] mb-[0.53vw] text-[#A1A1A1] uppercase'><Image src={el.icon} alt='icon'/>{el?.title}</h2>
                  <ul className='pl-[5.86vw] list-disc flex flex-col gap-[0.53vw] mb-[6.4vw]'>{
                    el?.list.map(i => <li key={i.id} className='text-[#2E2E2E]  font-semibold text-[3.73vw]'>{i.content}</li>)
                  }</ul>
              </div>
            ))}
          </div> : null )
      ...
      {show ? 'See less' : 'See more'}<Image src={rowUpImg} alt='arrow up' className={show ? '' : 'rotate-180'}/></button>
    </div>
  )
}
