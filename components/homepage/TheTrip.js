      ...
      <div className='flex flex-col md:gap-[0.75rem] gap-[4.27rem]'>
        ...
        <div className='flex md:gap-[5rem]'>
          {section8?.listCategory?.map((e, index) => (
            <div
              key={index}
              className={`${(isMobile && active === index) || !isMobile ? '' : 'hidden'} md:w-[26.375rem] w-full`}
            >
              <p className='md:text-[0.875rem] text-[3.73333rem] font-normal leading-[1.57] md:leading-[1.57] md:mb-[1.5rem] tracking-[0.00933rem] text-gray-scale-50'>
                {e?.description}
              </p>
