    ...
    return (
        <section className='mt-[6.25vw] px-[6.25vw] max-md:px-0'>
            <div className='flex flex-col items-center'>
                <h3 className='text-[#B34B1E] font-heavitas text-[1vw] leading-[1] max-md:text-[3.2vw]'>
                    {' '}
                    {valueTowards?.topTitle}{' '}
                </h3>
                <h2 className='text-[#B34B1E] font-heavitas text-[3vw] mt-[0.75vw] leading-[1] max-md:text-[6.4vw] max-md:mt-[2.1vw]'>
                    {' '}
                    {valueTowards?.title}{' '}
                </h2>
            </div>
            <div className='mt-[2.5vw] flex flex-col gap-[8.375vw] max-md:hidden'>
                {valueTowards?.content.map((item, index) => (
                    <div
                        className={`${index % 2 == 0 ? 'flex-row' : 'flex-row-reverse'} flex gap-[4.75vw]`}
                        key={index}
                    >
                        <Image
                            ...
                            width={688}
                            height={431}
                            className='w-[43vw] flex-1 h-[26.9375vw] rounded-[1vw] object-cover'
                        ></Image>
                        <div className='flex relative flex-1 items-start mt-[4.43vw]'>
                            <div
                                className='flex justify-center items-center w-[13.625vw] h-[13.625vw] rounded-[50%] flex-shrink-0'
                                style={{
                                    background:
                                        'linear-gradient(180deg, rgba(246, 185, 0, 0.60) 7.88%, rgba(255, 255, 255, 0.00) 98.49%)',
                                    backdropFilter: 'blur(1.6vw)',
                                }}
                            >
                                <Image
                                    ...
                                    width={218}
                                    height={218}
                                    className='h-[6.25vw] w-auto'
                                ></Image>
                            </div>
                            <div className='pt-[5vw] ml-[-2vw] relative z-[2]'>
                                <h3 className='text-[#05320C] text-[1.625vw] font-bold leading-[1.23] font-poppins'>
                                    {item?.title}
                                </h3>
                                <p className='mt-[1.25vw] text-[#727272] text-[0.875vw] leading-[1.57] tracking-[0.035] font-poppins'>
                                    {item?.content}
                                </p>
                            </div>
                            {index + 1 <= data.length - 1 && index % 2 === 0 && (
                                <svg
                                    className='absolute top-[13.625vw] right-[33.8vw] h-auto w-[46vw]'
                            ...
                            {index + 1 <= data.length - 1 && index % 2 !== 0 && (
                                <svg
                                    className='absolute top-[13.625vw] left-[6.6125vw] h-auto w-[46vw]'
                                    xmlns='http://www.w3.org/2000/svg'
                                    width='777'
                                    height='360'
                                    viewBox='0 0 777 360'
            ...
            <div className='md:hidden mt-[5.3vw]'>
                ...
                >
                    {dupData?.map((item, index) => (
                        ...
                        >
                            <div
                                className='flex justify-center items-center w-[34vw] h-[34vw] rounded-[50%]'
                                style={{
                                    background:
                                        index === indexSlider &&
                                        'linear-gradient(180deg, rgba(246, 185, 0, 0.60) 7.88%, rgba(255, 255, 255, 0.00) 98.49%)',
                                    backdropFilter: 'blur(6.9vw)',
                                }}
                            >
                                <Image
                                    src={item?.icon?.sourceUrl}
                                    ...
                                    width={128}
                                    height={128}
                                    className='w-auto h-[12.2vw] object-cover'
                                ></Image>
                            </div>
                            <h3 className='text-[#05320C] text-[3.7vw] font-bold leading-[1.23] font-poppins mt-[-4vw] text-center relative z-[2]'>
                                {item?.title}
                            </h3>
                        </SwiperSlide>
                    ))}
                ...
                >
                    {dupData.map((item, index) => (
                        <SwiperSlide
                            ...
                            key={index}
                        >
                            <p className='text-[#727272] text-[3.7vw] leading-[1.57] tracking-[0.035] font-poppins text-center'>
                                {item?.content}
                            </p>
                        </SwiperSlide>
                    ))}
