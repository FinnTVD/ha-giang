...
export default function Banner({bannerData, title}){
    return(
        <section className="relative pt-[11.25vw] pb-[4vw] max-md:px-0 max-md:pt-[49vw]">
            ...
            <div className={`${title?.includes('destinations') ? 'max-md:text-[11.2vw]' : 'max-md:text-[16.5vw]'} relative text-center font-heavitas text-[11.625vw] uppercase leading-[1]`} style={{backgroundImage: `url(${bannerData?.textBackground?.sourceUrl})`, backgroundRepeat: 'repeat',WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundPosition: 'center'}}>
                {title}
            </div>
        </section>
    )
