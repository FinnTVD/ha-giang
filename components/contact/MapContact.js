...
export default function MapContact({ data, footer }) {
  ...
  return (
    <section className='flex px-[6.25rem] h-fit gap-x-[1.5rem] mb-[6.25rem] max-md:flex-col-reverse max-md:px-[4.27rem] max-md:gap-y-[8rem] max-md:mt-[13rem]'>
      {/* <div className='relative w-fit h-fit'> */}
      <div
        id='boxMap'
        className='!w-[50.4275] max-md:!h-[90vh] max-lg:!w-[56rem] max-md:!w-full h-full rounded-[0.75rem] max-md:rounded-[4.267rem] overflow-hidden'
        dangerouslySetInnerHTML={{ __html: `${section1?.map}` }}
      />
      {/* <div className='absolute top-0 left-0 z-20 w-[7.0625rem] h-[2.25rem] rounded-[0.25rem] flex justify-center items-center bg-white'>
                    <Image
      ...
      <div className='flex-1'>
        <SubTitle
          boxClass={'flex flex-col-reverse text-center gap-y-[1rem] max-md:gap-y-[4.27rem]'}
